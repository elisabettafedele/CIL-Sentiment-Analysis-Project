...
def get_ensemble():
    ...
    classifiers.append(("rf", RandomForestClassifier(n_estimators = 200, max_depth = 10, min_samples_split = 30, n_jobs=-1)))
    classifiers.append(("svc", SGDClassifier(loss="log", penalty="l1")))
    classifiers.append(("xgbrf", XGBRFClassifier(n_estimators = 100, max_depth =  10, learning_rate = 0.1, min_child_weight =  1, gamma = 0, subsample = 0.7, colsample_bytree = 0.6, reg_alpha = 1e-5, tree_method = "gpu_hist", objective='binary:logistic', use_label_encoder=False)))
...
def get_models():
    ...
    classifiers["rf"] =  RandomForestClassifier(n_estimators = 200, max_depth = 10, min_samples_split = 30, n_jobs=-1)
    classifiers["sgd"] = SGDClassifier(loss="log", penalty="l1")
    classifiers["xgbrf"] =  XGBRFClassifier(n_estimators = 100, max_depth =  10, learning_rate = 0.1, min_child_weight =  1, gamma = 0, subsample = 0.7, colsample_bytree = 0.6, reg_alpha = 1e-5, tree_method = "gpu_hist", objective='binary:logistic', use_label_encoder=False)
